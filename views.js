import {state,active,characterViewFor} from "./state.js?v=20260805k";
import {eventFor,visibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260805k";
...
function relationshipMap(relations){
  ...
  const lines=edges.map((edge,index)=>{
    ...
    const officialMarkup=officialPoint?`<g class="map-official"><text class="map-relation" x="${officialPoint.x}" y="${officialPoint.y-4}" text-anchor="middle">${esc(relationText)}</text>${stageText?`<text class="map-stage" x="${officialPoint.x}" y="${officialPoint.y+15}" text-anchor="middle">${esc(stageText)}</text>`:""}</g>`:"";
